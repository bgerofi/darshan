...
/*add by shazb171318515@163.com*/
#include <sys/types.h>
#include <dirent.h>

DARSHAN_FORWARD_DECL(fdopendir, DIR*, (int fd));
DIR * DARSHAN_DECL(fdopendir)(int fd)
{
    DIR * ret;
    double tm1, tm2;
    ...
    ret = __real_fdopendir(fd);
    tm2 = darshan_core_wtime();

    POSIX_PRE_RECORD();
   

	struct posix_file_record_ref *rec_ref = darshan_lookup_record_ref(posix_runtime->fd_hash, &fd, sizeof(int));
	
	if(rec_ref){
		darshan_add_record_ref(&(posix_runtime->fd_hash), &ret, sizeof(DIR *), rec_ref);
		dxt_posix_write(rec_ref->file_rec->base_rec.id, -4, -4, tm1, tm2);
	}
...
DARSHAN_FORWARD_DECL(opendir, DIR*, (const char *name));
DIR * DARSHAN_DECL(opendir)(const char *name)
{
    DIR * ret;
    double tm1, tm2;
	...
	char *__newpath = darshan_clean_file_path(name);
    ...
    if(!__rec_ref)
		__rec_ref = posix_track_new_file_record(__rec_id, __newpath); 
    if(!__rec_ref) { 
        if(__newpath != name) 
			free(__newpath); 
    }else{
		darshan_add_record_ref(&(posix_runtime->fd_hash), &ret, sizeof(DIR *), __rec_ref);
		dxt_posix_write(__rec_ref->file_rec->base_rec.id, -1, -1, tm1, tm2);
	}
...
DARSHAN_FORWARD_DECL(readdir, struct dirent*, (DIR *dirp));
struct dirent * DARSHAN_DECL(readdir)(DIR *dirp)
{
    struct dirent *ret;
    double tm1, tm2;
	...
	struct posix_file_record_ref *rec_ref = darshan_lookup_record_ref(posix_runtime->fd_hash, &dirp, sizeof(DIR *));
	if(rec_ref){
		dxt_posix_write(rec_ref->file_rec->base_rec.id, -3, -3, tm1, tm2);
	}
    POSIX_POST_RECORD();
    return(ret);
}

DARSHAN_FORWARD_DECL(closedir, int, (DIR *dirp));
int DARSHAN_DECL(closedir)(DIR *dirp)
{
    int ret;
    double tm1, tm2;
	...
	struct posix_file_record_ref *rec_ref = darshan_lookup_record_ref(posix_runtime->fd_hash, &dirp, sizeof(DIR *));
	if(rec_ref){
		dxt_posix_write(rec_ref->file_rec->base_rec.id, -2, -2, tm1, tm2);
		darshan_delete_record_ref(&(posix_runtime->fd_hash), &ret, sizeof(DIR *));
	}
    POSIX_POST_RECORD();
    return(ret);
}

/*end add by shazb171318515@163.com*/
...
#define POSIX_RECORD_OPEN(__ret, __path, __mode, __tm1, __tm2) do { \
    ...
    /*add by shazb171318515@163.com*/ \
    dxt_posix_write(__rec_ref->file_rec->base_rec.id, -5, -5, __tm1, __tm2); \
    /*end add by shazb171318515@163.com*/ \
...
#define POSIX_RECORD_MY(__ret, __fd, __pwrite_flag, __pwrite_offset, __aligned, __tm1, __tm2) do { \
		struct posix_file_record_ref* rec_ref; \
		int64_t this_offset; \
		rec_ref = darshan_lookup_record_ref(posix_runtime->fd_hash, &__fd, sizeof(int)); \
		/* DXT to record detailed write tracing information */ \
...
int DARSHAN_DECL(close)(int fd)
{
    ...
    if(rec_ref)
    {
        ...
        DARSHAN_TIMER_INC_NO_OVERLAP(
            ...
            tm1, tm2, rec_ref->last_meta_end);
		/*add by shazb171318515@163.com*/
		dxt_posix_write(rec_ref->file_rec->base_rec.id, -6, -6, tm1, tm2); 
		/*end add by shazb171318515@163.com*/
